...
class EditSong extends Component {
    ...
    componentWillUpdate(nextProps) {
        debugger;
        if (nextProps.id) {
            //!this.state.selectedSong || (this.state.selectedSong && 
            if (this.state.selectedSong.id !== nextProps.id) {
                console.log('next id', nextProps.id);
                let songs = this.props.songses.filter(s => s.id === nextProps.id)[0];
                console.log(songs)
        ...
        if (this.state.selectedSong.id)
            son = (
                <div className="edit">
                    <form>
                        <label>שם</label>
                        <input className="inputs" id="name" type="text" value={this.state.selectedSong.name} onChange={(event) => this.inputChange(event)} />
                        <label >זמר</label>
                        <input className="inputs" id="USername" type="text" value={this.state.selectedSong.USername} onChange={(event) => this.inputChange(event)} />
                        <label>נוצר בשנת</label>
                        <input className="inputs" id="year" type="text" value={this.state.selectedSong.year} onChange={(event) => this.inputChange(event)} />
